
import React from 'react';
import {
  ContextMenu,
  ContextMenuContent,
  ContextMenuItem,
  ContextMenuTrigger,
} from '@/components/ui/context-menu';
import { MessageSquare, MessageCircle } from 'lucide-react';
import { SelectedTextContext } from '@/types/comments';

interface WritingContextMenuProps {
  children: React.ReactNode;
  onComment: (position: { x: number; y: number }, selectedText?: SelectedTextContext, lineNumber?: number) => void;
  onChat: (position: { x: number; y: number }) => void;
}

const WritingContextMenu = ({
  children,
  onComment,
  onChat
}: WritingContextMenuProps) => {
  const [contextPosition, setContextPosition] = React.useState({ x: 0, y: 0 });

  const handleContextMenu = (event: React.MouseEvent) => {
    console.log('Context menu triggered at:', event.clientX, event.clientY);
    setContextPosition({ x: event.clientX, y: event.clientY });
    
    // Calculate line number from cursor position in textarea
    const target = event.target as HTMLTextAreaElement;
    if (target && target.tagName === 'TEXTAREA') {
      const textBeforeCursor = target.value.substring(0, target.selectionStart);
      const lineNumber = textBeforeCursor.split('\n').length;
      
      // Store line number for context menu actions
      (window as any).currentLineNumber = lineNumber;
      console.log('Line number at cursor:', lineNumber);
    }
  };

  const getSelectedTextContext = (): SelectedTextContext | null => {
    return (window as any).selectedTextContext || null;
  };

  const getCurrentLineNumber = (): number | undefined => {
    return (window as any).currentLineNumber;
  };

  const handleCommentClick = (event: React.MouseEvent) => {
    event.preventDefault();
    event.stopPropagation();
    console.log('Comment clicked, position:', contextPosition);
    const selectedText = getSelectedTextContext();
    const lineNumber = getCurrentLineNumber();
    onComment(contextPosition, selectedText, lineNumber);
  };

  const handleChatClick = (event: React.MouseEvent) => {
    event.preventDefault();
    event.stopPropagation();
    console.log('Chat clicked, position:', contextPosition);
    onChat(contextPosition);
  };

  const selectedText = getSelectedTextContext();

  return (
    <ContextMenu>
      <ContextMenuTrigger 
        className="h-full w-full"
        onContextMenu={handleContextMenu}
        asChild
      >
        <div className="h-full w-full">
          {children}
        </div>
      </ContextMenuTrigger>
      <ContextMenuContent className="w-56 bg-white border border-slate-200 shadow-lg z-[10000]">
        <ContextMenuItem 
          onClick={handleCommentClick}
          className="flex items-center gap-2 px-3 py-2 hover:bg-slate-50 cursor-pointer"
          onSelect={(e) => e.preventDefault()}
        >
          <MessageSquare className="w-4 h-4 text-blue-600" />
          <div className="flex flex-col">
            <span>Add Comment</span>
            {selectedText && (
              <span className="text-xs text-slate-500 truncate max-w-40">
                on "{selectedText.text}"
              </span>
            )}
          </div>
        </ContextMenuItem>
        <ContextMenuItem 
          onClick={handleChatClick}
          className="flex items-center gap-2 px-3 py-2 hover:bg-slate-50 cursor-pointer"
          onSelect={(e) => e.preventDefault()}
        >
          <MessageCircle className="w-4 h-4 text-orange-600" />
          <span>Open Chat Assistant</span>
        </ContextMenuItem>
      </ContextMenuContent>
    </ContextMenu>
  );
};

export default WritingContextMenu;
